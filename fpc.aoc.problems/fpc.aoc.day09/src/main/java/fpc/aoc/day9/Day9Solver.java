...
public abstract class Day9Solver extends SmartSolver<Stream<Command>,Integer> {

    @Override
    protected @NonNull Converter<Stream<Command>> getConverter() {
        return s -> s.map(Command::parse);
    }
    ...
    @Override
    public @NonNull Integer solve(@NonNull Stream<Command> commands) {
