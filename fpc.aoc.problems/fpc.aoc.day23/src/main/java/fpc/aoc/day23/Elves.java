...
public class Elves {
  ...
  public Elves(ImmutableSet<Position> positions) {
  ...
  public Elves performOneRound() {
    ...
    final var builder = ImmutableSet.<Position>builder();
    for (Map.Entry<Position, List<Position>> entry : propositions.first().entrySet()) {
      ...
      if (oldPositions.size() == 1) {
        builder.add(newPosition);
      } else {
        builder.addAll(oldPositions);
      }
    }

    return new Elves((directionOffset + 1) % DIRECTIONS.length, builder.build(),propositions.second() == positions.size());
  }
